...
- **☁️ 线上数据同步**：将模型加入队列时，自动异步拉取线上真实的 Avatar 名称与封面缩略图（所见即所得），彻底杜绝本地命名混乱导致的误覆盖他人模型。
- **🆔 蓝图安全管理**：实时拦截重复的蓝图 ID，并支持在面板内一键剥离（Detach）复用模型的 ID，无需繁琐切换即可无缝转为全新模型上传。
- **👁️ 场景深度联动**：支持按 Hierarchy 层级一键重排乱序的队列；支持开启“上传成功后自动隐藏场景对象”，让一觉醒来后的查错工作一目了然。
- **🛡️ 极致容错预检**：构建前毫秒级拦截包含 `Missing Script` 的异常模型，避免 SDK 中途卡死；支持一键复制失败日志并高亮场景中出错的对象。
- **📱 远程战报推送**：支持配置 Discord、飞书或 Slack 的 Webhook URL。挂机任务完成后，系统会自动向你的手机推送包含成功/失败数与总耗时的极简战报。
- **🌐 多语言原生支持**：内置 简体中文 / 日本語 / English / 한국어 四语界面，可在设置中随时切换并自动保存偏好。
...
2. **添加任务**：
  ...
  - 或者直接点击工具栏的 [扫描场景]。
3. **调整队列**：你可以通过点击行尾的 `▲` `▼` 调整上传顺序、点击工具栏的排序按钮按层级重排，或点击 `↑` 将某个模型设为紧急优先。
4. **个性化设置**：在设置页面 (Settings) 中，你可以切换语言、调整缓冲倒计时、配置 Webhook 推送地址，以及开关各种自动化设定。
...
> 我是逻辑XR。随着工具核心机制的完善与 API 的深度接入，本插件目前已经基本实现了我最初构想的所有核心痛点优化。
> 
> **项目现已度过快速迭代期，后续将进入缓慢更新与平稳维护阶段。**
> 如果你在使用过程中遇到任何 Bug，或者有绝佳的新功能点子，非常欢迎随时在 GitHub 提交 Issue 探讨，我会定期查看并维护。感谢大家的支持！
